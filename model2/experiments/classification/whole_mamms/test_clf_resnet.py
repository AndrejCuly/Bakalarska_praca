...
Evaluates best_model_resnet_clf.pth on the held-out test set.
Reports AUC-ROC, accuracy, sensitivity, specificity, confusion matrix.
Saves per-patient predictions to CSV and ROC curve to PNG.
"""
...
BATCH_SIZE   = 8
NUM_WORKERS  = 4
RESULTS_CSV  = os.path.join(os.path.dirname(__file__), 'results', 'test_results_resnet.csv')
ROC_PLOT     = os.path.join(os.path.dirname(__file__), 'results', 'roc_curve_resnet.png')
...
def main():
    ...
    print("\n" + "="*50)
    print("TEST SET RESULTS — Model 2 (MedicalNet ResNet-10)")
    print("="*50)
    ...
    plt.title('ROC Curve — Model 2 MedicalNet ResNet-10 (Whole Mammograms)')
