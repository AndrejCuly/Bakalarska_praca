...
# Which classifiers output probability (True) vs binary class (False)
IS_REGRESSION = {
    "Decision Tree":       False,
    "Random Forest":       False,
...
def evaluate(name, clf, X_test, y_test, patients, ax=None):
    ...
    # For classification experiments use hard predictions
    # For regression experiments report probability as output
    is_regression = IS_REGRESSION.get(name, False)
    if is_regression:
        # Use 0.5 threshold only for computing sensitivity/specificity
        preds = (probs >= 0.5).astype(int)
        output_col = 'predicted_prob'
    else:
        preds = clf.predict(X_test)
        output_col = 'predicted_class'
...
def main():
    ...
    X_train, y_train, p_train = load_split("train")
    X_val,   y_val,   p_val   = load_split("val")
    ...
    classifiers = [
        ...
        ("Random Forest",
         ...
         False),
        ("Logistic Regression",
         LogisticRegression(C=0.1, class_weight=cw, max_iter=1000, random_state=42),
         True),
        ("Gradient Boosting",
