"""
extract_features_resnet50_patches.py

Extracts 256-dim feature vectors from the frozen ResNet50 3D encoder
for patch classification (Model 4).

Run this BEFORE train_hybrid_resnet50_patches.py.
Saves features to model4/experiments/decision_tree_hybrid/patches/features_patches/
"""
...
CHECKPOINT    = r"C:\Skola\Bakalarka\Model1\default\pngs_processed\model4\experiments\classification\patches\results\best_model_resnet50_clf_patches.pth"
...
def main():
    ...
    splits = [
        ("train", CANCER_TRAIN,      CANCER_FREE_TRAIN),
        ("val",   CANCER_VAL,        CANCER_FREE_VAL),
        ("test",  CANCER_TEST,       CANCER_FREE_TEST),
    ]

    for split_name, canc_dir, free_dir in splits:
        dataset = PatchClassificationDataset(
            cancer_dir=canc_dir, cancer_free_dir=free_dir)
