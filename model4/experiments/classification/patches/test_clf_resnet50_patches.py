"""
test_clf_resnet50.py

Evaluates best_model_resnet50_clf.pth on the held-out test set.
Model 4 (ResNet50 3D) — whole mammograms, 512x512.
"""
...
# -------------------------------------------------------------------------
CANCEROUS_TEST   = r"C:\Users\culya\Desktop\data_bakalarka\data\patches_split\test\cancer"
CANCER_FREE_TEST = r"C:\Users\culya\Desktop\data_bakalarka\data\patches_split\test\cancer_free"
CHECKPOINT  = os.path.join(os.path.dirname(__file__), 'results', 'best_model_resnet50_clf_patches.pth')
BATCH_SIZE  = 4
NUM_WORKERS = 8
...
def main():
    ...
    test_dataset = PatchClassificationDataset(
        cancer_dir=CANCEROUS_TEST, cancer_free_dir=CANCER_FREE_TEST)
    ...
    model = MammogramResNet50Classifier(num_views=4, embed_dim=256).to(device)
    ...
    print("\n" + "="*50)
    print("TEST SET RESULTS — Model 4 ResNet50 3D (Patches)")
    print("="*50)
    ...
    plt.title('ROC Curve — Model 4 ResNet50 3D (Whole Mammograms)')
