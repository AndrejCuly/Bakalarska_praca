"""
train_hybrid_patches.py

Trains and evaluates classical ML classifiers on top of the 256-dim
features extracted from the frozen patches 3D CNN encoder.

Classifiers:
    - Decision Tree    (Experiment 3b)
    - Random Forest    (Experiment 4b)
    - Logistic Regression (Experiment 5b)
    - Gradient Boosting   (Experiment 6b)
"""
...
ROC_PLOT     = os.path.join(os.path.dirname(__file__), 'results/roc_curve_hybrid_patches.png')
RESULTS_FILE = os.path.join(os.path.dirname(__file__), 'results/hybrid_patches_results.txt')
...
def main():
    ...
    classifiers = [
        ("Decision Tree",
         DecisionTreeClassifier(max_depth=5, class_weight=cw, random_state=42),
         False),
        ...
        ("Logistic Regression",
         LogisticRegression(C=0.1, class_weight=cw,
    ...
    with open(RESULTS_FILE, 'w') as f:
        ...
        f.write("="*60 + "\n\n")
        f.write(f"Train+Val samples: {len(X_trainval)} "
